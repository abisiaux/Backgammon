...
void Display_Die(SDisplay* display, SGameState* gameState, SGame* game)
{
	...
	else // Sinon simple affichage des deux dés
	{
		...
		if(game->die_To_Play[1]) // S'il n'a pas été déjà joué
			...
			SDL_BlitSurface(display->die_played[gameState->die2-1], NULL,display->screen, &display->die2_position);
	}
}
...
{
	if(gameState->zones[move->src_point-1].nb_checkers == 0)
		return ;
	...
	if(move->dest_point>12 && move->dest_point!=EPos_OutP1 && move->dest_point!=EPos_OutP2)
	{
	...
	{
		int sens = 1;
		if(gameState->zones[move->src_point-1].player == EPlayer2)
			sens = -1;
		
			pos_dest_y = display->positions[move->dest_point-1].y + sens*20*(gameState->zones[move->dest_point-1].nb_checkers);
	}
	...
	move->dest_point += -1;
	move->src_point += -1;
	updateDieStatue(move, gameState, game); // On met à jour le statut des des
...
int CheckerWithScreenPosition(int x, int y, EPosition *pos)
{
	...
	else if(x>=295 && x<=320) // Les barres
	{
		if(y>=370 && y<=520)// Barre du Joueur1
		{
		
			*pos=EPos_BarP1;
			return 1;
		}
		else if(y>=220 && y<=350)// Barre du Joueur2
		{
			*pos=EPos_BarP2;
			return 1;
		}
	}
	else if(x>=560 && x<=615) // Les sorties
	{
		if(y>=140 && y<=260)// Sortie du Joueur1
		{
			*pos=EPos_OutP1;
			return 1;
		}
		else if(y>=480 && y<=600)// Barre du Joueur2
		{
			*pos=EPos_OutP2;
			return 1;
		}
...
void colorChecker(SDisplay* display, SGameState* game, EPosition pos)
{
	...
	else if(pos == EPos_BarP1)
	{
		posSelectedChecker.y -= (selectedZone.nb_checkers)*10; //mise a jour de y pour selectionner le pion du haut
	}
	else if(pos == EPos_BarP2)
	{
		posSelectedChecker.y += (selectedZone.nb_checkers)*10; //mise a jour de y pour selectionner le pion du haut
	}
...
int possibleMove(SGameState* gameState, EPlayer player, EPosition p, SGame* game)
{
	int sens = 1;
	SMove tmp;
	EPosition sortie = EPos_OutP1;
	if(player == EPlayer2)
	{
		sens = -1;
		sortie = EPos_OutP2;
	...
	{
		tmp.src_point = p;
		tmp.dest_point = p + sens*gameState->die1; // Test avec die1
		if(gameState->zones[tmp.dest_point].nb_checkers == 0 || (gameState->zones[tmp.dest_point].player == player) || (gameState->zones[tmp.dest_point].player != player && gameState->zones[tmp.dest_point].nb_checkers == 1))
		{
			if(tmp.dest_point >= sortie)
			{
				tmp.dest_point = sortie;
				if(allInJan(gameState,player) && (game->die_To_Play[0]==1))
					return 1;
			}
			else if(game->die_To_Play[0]==1)
				return 1;
		}
		tmp.dest_point = p + sens*gameState->die2; // Test avec die2
		if(gameState->zones[tmp.dest_point].nb_checkers == 0 || (gameState->zones[tmp.dest_point].player == player) || (gameState->zones[tmp.dest_point].player != player && gameState->zones[tmp.dest_point].nb_checkers == 1))
		{
			if(tmp.dest_point >= sortie)
			{
				tmp.dest_point = sortie;
				if(allInJan(gameState,player) && (game->die_To_Play[1]==1))
					return 1;
			}
			else if(game->die_To_Play[1]==1)
				return 1;
		}
		tmp.dest_point = p + sens*(gameState->die1 + gameState->die2); // Test avec die1 + die2
		if(gameState->zones[tmp.dest_point].nb_checkers == 0 || (gameState->zones[tmp.dest_point].player == player) || (gameState->zones[tmp.dest_point].player != player && gameState->zones[tmp.dest_point].nb_checkers == 1))
		{
			if(tmp.dest_point >= sortie)
			{
				tmp.dest_point = sortie;
				if(allInJan(gameState,player) && (game->die_To_Play[0]==1 && game->die_To_Play[1]==1))
					return 1;
			}
			else if(game->die_To_Play[0]==1 && game->die_To_Play[1]==1)
				return 1;
		}
	}
	else // Si double
	{
		...
		switch(nbDesRestant)
		{
			case 1:
				tmp.src_point = p;
				tmp.dest_point = p + sens*gameState->die1;
				if(gameState->zones[tmp.dest_point].nb_checkers == 0 || (gameState->zones[tmp.dest_point].player == player) || (gameState->zones[tmp.dest_point].player != player && gameState->zones[tmp.dest_point].nb_checkers == 1))
				{
					if(tmp.dest_point >= sortie)
					{
			...
			case 2:
				...
				tmp.src_point = p;
				tmp.dest_point = p + sens*gameState->die1*2;
				if(gameState->zones[tmp.dest_point].nb_checkers == 0 || (gameState->zones[tmp.dest_point].player == player) || (gameState->zones[tmp.dest_point].player != player && gameState->zones[tmp.dest_point].nb_checkers == 1))
				{
					if(tmp.dest_point >= sortie)
					{
			...
			case 3:
				...
				tmp.src_point = p;
				tmp.dest_point = p + sens*gameState->die1*3;
				if(gameState->zones[tmp.dest_point].nb_checkers == 0 || (gameState->zones[tmp.dest_point].player == player) || (gameState->zones[tmp.dest_point].player != player && gameState->zones[tmp.dest_point].nb_checkers == 1))
				{
					if(tmp.dest_point >= sortie)
					{
			...
			case 4 :
				...
				tmp.src_point = p;
				tmp.dest_point = p + sens*gameState->die1*4;
				if(gameState->zones[tmp.dest_point].nb_checkers == 0 || (gameState->zones[tmp.dest_point].player == player) || (gameState->zones[tmp.dest_point].player != player && gameState->zones[tmp.dest_point].nb_checkers == 1))
				{
					if(tmp.dest_point >= sortie)
					{
...
int src_isValid(SGameState* gameState, EPosition src, EPlayer player)
{
	return (	src != EPos_nopos &&
				src != EPos_OutP1 &&
				src != EPos_OutP2 &&
				gameState->zones[src].nb_checkers>0 &&
				gameState->zones[src].player == player
			);
}

int allDieUsed(SGameState* gameState, SGame* game)
{
	int i;
	int nbDie = 2;
	if(gameState->die1 == gameState->die2) nbDie = 4;
...
int Display_CheckersPossibilities(SDisplay* display, SGameState* gameState, EPlayer player, SGame* game)
{
	int i;
	int nbtour = 1;
	if (!allDieUsed(gameState,game))
	{
		printf("Tour n%d\n",nbtour);
		nbtour++;
		int* arriveesValides = NULL;
		int posDepartSelected = 0;
		SDL_Event event;

		EPosition prison = EPos_BarP1;

		int tab[28]; // Tableau contenant les pions bougeables
		int nbPions = 0; // Nombre de pions bougeables

		for(i=0;i<28;i++) tab[i]=-1; // Init du tableau


		if(player == EPlayer2)
			prison = EPos_BarP2;
	
		// Si il y a déjà des prisonniers
		if(gameState->zones[prison].nb_checkers>0)
		{
			if(possibleMove(gameState,player,prison,game))
			{
				colorChecker(display,gameState,prison);
				tab[nbPions]=prison;
				nbPions++;
				printf("Possibilité de sortir prisonnier ! \n");
			}
			else
			{
		...
		else // S'il n'y a pas de prisonnier
		{
			for(i=0;i<24;i++)
			{
				if(src_isValid(gameState, (EPosition)i,player) && possibleMove(gameState,player,(EPosition)i,game))
				{
					colorChecker(display,gameState,(EPosition)i);
					tab[nbPions]=i;
					nbPions++;
				}
			}
				...
				case SDL_MOUSEBUTTONUP:
					if (event.button.button == SDL_BUTTON_LEFT)
					{
						...
						if(!posDepartSelected) // selection de la position depart
						{
							...
							{
								printf("Position posDepart = %d trouvee!\n",posDepart);
								//for(i=0;i<28;i++) printf("Pion bougeable %d\n",tab[i]);
								if(inTab(posDepart, tab))
								{
									printf("SELECT POS DEPART\n");
						...
						else // selection de la position arrivée
						{
							...
							{
								printf("POS ARRIVEE\n");
								if(arriveesValides!=NULL && inTab(posArrivee, arriveesValides))
								{
...
void updateDieStatue(SMove* mouvement, SGameState* gameState, SGame* game)
{
	...
	unsigned int taille_mouvement = Arbitrator_Get_Distance(mouvement->src_point,mouvement->dest_point);
	int nbDie = 2;
	if(gameState->die1 == gameState->die2)
		nbDie = 4;
	...
	if(nbDie == 2)
	{
		printf("Jeu avec 2 dés\n");
		printf("Taille mouvement = %d\n",taille_mouvement);
		if(taille_mouvement == gameState->die1 && game->die_To_Play[0] != 0) // taille du mouvement correspond au dé 1
		{
			printf("Jeu avec dé 1\n");
			game->die_To_Play[0] = 0;	
		}
		else if(taille_mouvement == gameState->die2 && game->die_To_Play[1] != 0) // taille du mouvement correspond au dé 2
		{
			printf("Jeu avec dé 2\n");
			game->die_To_Play[1] = 0;	
		}
		else if(taille_mouvement == (gameState->die1+gameState->die2) && game->die_To_Play[0] != 0 && game->die_To_Play[1] != 0)// taille du mouvement correspond au dé1 + dé2
		{
			printf("Jeu avec dé 1 et dé2\n");
			game->die_To_Play[0] = 0;
			game->die_To_Play[1] = 0;
		}
	}
	...
	{
		int nbDieForMove = taille_mouvement / gameState->die1;
		printf("Nb  dés pour mouvement = %d\n",nbDieForMove);
		int i;
		printf("Jeu avec 4 dés\n");
		if(nbDieForMove == 1) // taille du mouvement correspond à un dés
		{
			i=0;
			while((game->die_To_Play[i]==0) & (i<4)) i++;
			game->die_To_Play[i] = 0;	
		}
		else if(nbDieForMove == 2) // taille du mouvement correspond à deux dés
		{
			i=0;
			while(game->die_To_Play[i]==0) i++;
			game->die_To_Play[i] = 0;
			i++;
			while(game->die_To_Play[i]==0 && i<4) i++;
			game->die_To_Play[i] = 0;
		...
		else if(nbDieForMove == 3) // taille du mouvement correspond à trois dés
		{
			i=0;
			while(game->die_To_Play[i]==0) i++;
			game->die_To_Play[i] = 0;
		...
		else // taille du mouvement correspond à quatre dés
		{
			i=0;
			while(game->die_To_Play[i]==0) i++;
			game->die_To_Play[i] = 0;
...
int* Display_Arrow_Possibilities(SDisplay *display, SGameState *gameState, EPlayer player, EPosition p, SGame *game)
{
	Display_Clear(display);
	Display_RefreshGameBoard(display, gameState, game);
	colorChecker(display,gameState,p);
	
	int *arriveesPossibles = NULL;
	...
	if(gameState->die1 != gameState->die2)
	{
		...
		if(gameState->zones[tmp.dest_point].nb_checkers == 0 || (gameState->zones[tmp.dest_point].player == player) || (gameState->zones[tmp.dest_point].player != player && gameState->zones[tmp.dest_point].nb_checkers == 1))
		{
			if(tmp.dest_point >= sortie)
			{
				...
				if(allInJan(gameState,player) && (game->die_To_Play[0]==1))
				{
					arriveesPossibles[cpt] = tmp.dest_point;
					Display_DrawSelectedArrow(display, tmp.dest_point);
					cpt++;
				}
			}
			else if(game->die_To_Play[0]==1)

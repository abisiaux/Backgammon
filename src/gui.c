...
{	
	...
	strcpy(temp_path, display->img_path);
	strcat(temp_path, "bar_white_checker.png");
	display->bar_white_checker = IMG_Load(temp_path);
	
	// Chargement de l'image de pion vert pour les barres
	strcpy(temp_path, display->img_path);
	strcat(temp_path, "bar_green_checker.png");
	display->bar_green_checker = IMG_Load(temp_path);
...
void Display_Exit(SDisplay *display)
{
	...
	SDL_FreeSurface(display->bar_white_checker);
	SDL_FreeSurface(display->bar_green_checker);
...
void Display_DrawOut(SDisplay *display, SGameState *gameState, SGame *game)
{
	...
	for(i=0;i<gameState->zones[24].nb_checkers;i++)
	{
		pos.y += 10;
		if(game->player1_checker == GREEN)
			SDL_BlitSurface(display->bar_green_checker, NULL,display->screen, &pos);
		else
			SDL_BlitSurface(display->bar_white_checker, NULL,display->screen, &pos);
	}
		...
		for( j=0; j<gameState->zones[i].nb_checkers; j++)
		{
			...
			else
			{
				...
				if( i<12)
					...
					new_pos.y = display->positions[i].y + j*20;
				Checker_Draw(display,new_pos,gameState->zones[i].player,game);
...
void Checker_Move(SDisplay *display, SGameState* gameState, SMove *move, SGame *game)
{
	...
	while( !depassement )
	{
		...
		Checker_Draw(display,new_pos,gameState->zones[move->src_point-1].player,game);
		SDL_Flip(display->screen);
...
int* Display_Arrow_Possibilities(SDisplay *display, SGameState *gameState, EPlayer player, EPosition depart, SGame *game)
{
	...
	if(gameState->die1 != gameState->die2) // Si dés différents
	{
		...
		if(ArriveeAvecDE1.player == player ||  ArriveeAvecDE1.nb_checkers == 0 || (ArriveeAvecDE1.player != player &&  ArriveeAvecDE1.nb_checkers == 1) )
		{
			...
			Draw_Selected_Arrow(display, depart+(gameState->die1)*sens);
			arrowWithDie1 = 1;
		...
		if(ArriveeAvecDE2.player == player ||  ArriveeAvecDE2.nb_checkers == 0 || (ArriveeAvecDE2.player != player &&  ArriveeAvecDE2.nb_checkers == 1))
		{
			...
			Draw_Selected_Arrow(display, depart+(gameState->die2)*sens);
		...
		if( (ArriveeAvecDE1ETDE2.player == player ||  ArriveeAvecDE1ETDE2.nb_checkers == 0 || (ArriveeAvecDE1ETDE2.player != player &&  ArriveeAvecDE1ETDE2.nb_checkers == 1 )) && (arrowWithDie1 || arrowWithDie2) )
		{
			...
			Draw_Selected_Arrow(display, depart+(gameState->die1)*sens+(gameState->die2)*sens);
	...
	else
	{
		...
		for(int i=1;i<=4;i++)
		{
			...
			if((ArriveeAvecDE.player == player ||  ArriveeAvecDE.nb_checkers == 0 || (ArriveeAvecDE.player != player ||  ArriveeAvecDE.nb_checkers == 1)) && (nbpossibilite==i))
			{
				...
				Draw_Selected_Arrow(display, depart+((gameState->die1)*sens*i));
...
int Display_GameActions(SDisplay *display, SGameState* gameState, SGame *game)
{
	SDL_Rect pos;
	int notSelected = 1;
	SDL_Surface *erase = SDL_CreateRGBSurface(SDL_HWSURFACE, 160, 45, 32, 0, 0, 0, 0);
	pos.x = 630;
	pos.y = 190;
	if(game->withDouble)
	{
	...
	while(1)
	{
		...
		switch(event.type)
		{
			...
			case SDL_MOUSEBUTTONUP:
				if (event.button.button == SDL_BUTTON_LEFT)
				{
					if((event.button.x>=630 && event.button.x<=790 && event.button.y>=190 && event.button.y<=235) && game->withDouble && notSelected) // Double
					{
					...
					if((event.button.x>=630 && event.button.x<=790 && event.button.y>=190 && event.button.y<=235) && game->withDouble) // Lancer
					{
...
int Display_CheckersPossibilities(SDisplay *display, SGameState *gameState, EPlayer player, SGame *game, int Die_For_Play[7])
{
	...
	for(i=0;i<28;i++)
	{
		if(gameState->zones[i].player == player && gameState->zones[i].nb_checkers>0) // Si les pions sur la flèche appartiennent au joueur
		{
			...
			tmp.dest_point = (EPosition)i+sens*gameState->die1;
			if(authorized_deplacement(gameState,&tmp, player, Die_For_Play)) // Si le déplacement flèche est autorisé
			{
				mat[i][i+sens*gameState->die1] = 1;
				printf("MOUVEMENT %d => %d POSSIBLE\n",tmp.src_point,tmp.dest_point);
				noneMove = 0;
			}
			tmp.dest_point = (EPosition)i+sens*gameState->die2;
			if(authorized_deplacement(gameState,&tmp, player, Die_For_Play)) // Si le déplacement flèche est autorisé
			{
				mat[i][i+sens*gameState->die1] = 1;
				printf("MOUVEMENT %d => %d POSSIBLE\n",tmp.src_point,tmp.dest_point);
			...
			tmp.dest_point = (EPosition)i+sens*(gameState->die1+gameState->die2);
			if(authorized_deplacement(gameState,&tmp, player, Die_For_Play)) // Si le déplacement flèche est autorisé
			{
				mat[i][i+sens*gameState->die1] = 1;
				printf("MOUVEMENT %d => %d POSSIBLE\n",tmp.src_point,tmp.dest_point);
	...
	if( noneMove) // Si pas de mouvement possible on affiche un message
	{
		SDL_Rect msg_position;
		msg_position.x = 80;
		msg_position.y = 325;
	...
	else // Sinon On appelle la fonction pour choisir le mouvement
	{
		quit = makeChoice(display, gameState, game, mat, player);
	}
	return quit;
}
...
int nbDesRestant(SGame* game)
{
	return 0;
}
int makeChoice(SDisplay *display, SGameState *gameState, SGame *game, int mat[28][28], EPlayer player)
{
	SDL_Event event;
	int posDepartSelected = 0;
	...
	while(1)
	{
		...
		switch(event.type)
		{
			...
			case SDL_MOUSEBUTTONUP:
				...
				{
					//Si il y a clic sur un autre pion de départ cliqué, malgré qu'un pion de départ ait déja été choisi avant
					if( posDepartSelected && CheckerWithScreenPosition(event.button.x, event.button.y, &posDepart) && src_valid(mat,posDepart))
					{
						posDepartSelected = !posDepartSelected;
					}
					...
					if(!posDepartSelected) // selection de la position depart
					{
						if(CheckerWithScreenPosition(event.button.x, event.button.y, &posDepart)) // il a trouvé une position valide 
						{
							...
							{
								Display_Arrow_Possibilities(display, gameState, player, posDepart, game);
								posDepartSelected = 1;
								mouvement.src_point = posDepart+1;
							}
						}
					...
					else // selection de la position arrivée
					{
						...
						{
							Display_RefreshGameBoard(display, gameState, game);
							return makeChoice(display, gameState, game, mat, player);
						}
					}
				}
				break;

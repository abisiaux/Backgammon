...
{	
	...
	// Bar Player 1
	display->positions[24].x = 292;
	display->positions[24].y = 480;
	// Out Player 1
	display->positions[25].x = 568;
	display->positions[25].y = 140;
	// Bar PLayer 2
	display->positions[26].x = 292;
...
void Display_DrawBar(SDisplay *display, SGameState *gameState, SGame *game)
{
	...
	for(i=0;i<gameState->zones[24].nb_checkers;i++) // Barre du joueur 1
	{
		pos.y -= 10;
		if(game->player1_checker == GREEN)
	...
	for(i=0;i<gameState->zones[26].nb_checkers;i++) // Barre du joueur 2
	{
		pos.y += 10;
		if(game->player2_checker == GREEN)
			SDL_BlitSurface(display->green_checker, NULL,display->screen, &pos);

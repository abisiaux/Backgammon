...
{	
	...
	// Bar Player 1
	display->positions[24].x = 304;
	display->positions[24].y = 511;
	// Out Player 1
	display->positions[25].x = 568;
	display->positions[25].y = 140;
	// Bar PLayer 2
	display->positions[26].x = 304;
	display->positions[26].y = 220;
	// Out Player 2
	display->positions[27].x = 568;
	display->positions[27].y = 590;
...
void Display_RefreshGameBoard(SDisplay *display, SGameState *gameState, SGame *game)
{
	...
	DrawBar(display, gameState, game);
...
void DrawBar(SDisplay *display, SGameState *gameState, SGame *game)
{
	...
	for(i=0;i<gameState->zones[25].nb_checkers;i++)
	{
		pos.y += 10;
		if(game->player1_checker == GREEN)
	...
	for(i=0;i<gameState->zones[27].nb_checkers;i++)
	{
		pos.y -= 10;
		if(game->player2_checker == GREEN)
			SDL_BlitSurface(display->bar_green_checker, NULL,display->screen, &pos);
...
int Display_CheckersPossibilities(SDisplay *display, SGameState *gameState, EPlayer player, SGame *game)
{
	...
	if(gameState->die1 != gameState->die2) // Si dés différents
	{
		for( i=0; i<24; i++) // Pour chaque flèche
		{
			...
			{
				if( (i+gameState->die1)<24 && (gameState->zones[i+gameState->die1].nb_checkers==0 || gameState->zones[i+gameState->die1].player==player) )
				{
				...
				if( (i+gameState->die2)<24 && (gameState->zones[i+gameState->die2].nb_checkers==0 || gameState->zones[i+gameState->die2].player==player) )
				{
				...
				if( (i+gameState->die1+gameState->die2)<24 && (gameState->zones[i+gameState->die1+gameState->die2].nb_checkers==0 || gameState->zones[i+gameState->die1+gameState->die2].player==player) )
				{

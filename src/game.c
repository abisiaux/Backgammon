...
SGameState* Game_Init()
{
	...
	game->zones[25].player=EPlayer1;
	game->zones[26].player=EPlayer2;
	game->zones[27].player=EPlayer2;
	return game;
}
			...
			switch (gameMode)
			{
				...
				case HUMAN_IA :
					...
					else
					{
						...
						for(i=0;i<4;i++)
						{
							...
							{
								printf("AI JOUE src = %d dest = %d\n",mvmt_ia[i].src_point,mvmt_ia[i].dest_point);
								convertMove(&(mvmt_ia[i]),curentP);
								printf("AI JOUE src = %d dest = %d\n",mvmt_ia[i].src_point,mvmt_ia[i].dest_point);
				...
				case IA_IA :
					if(curentP == EPlayer1)
					{
						...
						for(i=0;i<4;i++)
						{
							if(mvmt_ia[i].src_point != EPos_nopos && mvmt_ia[i].dest_point != EPos_nopos && !allDieUsed(gameState,game))
							{
								printf("AI %d JOUE src = %d dest = %d\n",curentP,mvmt_ia[i].src_point,mvmt_ia[i].dest_point);
								convertMove(&(mvmt_ia[i]),curentP);
								printf("AI %d JOUE src = %d dest = %d\n",curentP,mvmt_ia[i].src_point,mvmt_ia[i].dest_point);
								/*if(Arbitrator_AuthorizedDeplacement(gameState, mvmt_ia, curentP, game))
								{*/
					...
					else
					{
						...
						copyGameS = copyGameState(gameState,curentP);
						(*ia_struct[1].IA_MakeDecision)(copyGameS, mvmt_ia, lastTimeError); 
						for(i=0;i<4;i++)
						{
							if(mvmt_ia[i].src_point != EPos_nopos && mvmt_ia[i].dest_point != EPos_nopos && !allDieUsed(gameState,game))
							{
...
void convertMove(SMove* move,EPlayer player)
{
	if(player == EPlayer2)
	{
		...
		{
			move->src_point = EPos_BarP2;
		}
		// Si c'est une sortie
		if( move->src_point == EPos_OutP1)
		{
			move->dest_point = EPos_OutP2;
		}
	}
	else
	{
		...
		if( move->src_point == EPos_BarP1)
		{
			move->dest_point = 24 - move->dest_point;
		}
		// Si c'est une sortie
		else if( move->src_point == EPos_OutP1)
		{
			move->dest_point = EPos_OutP2;
		}
		else
		{
			//tmp = move->src_point;
			//move->src_point = 24 - move->dest_point;
			//move->dest_point = tmp;
			/*if(move->src_point <= 12)
				move->dest_point = 24 -move->dest_point;
			/*else if(move->src_point > 12)
				move->dest_point = 24 -move->dest_point;*/
			if(move->src_point > move->dest_point)
				move->dest_point = move->src_point + (move->src_point - move->dest_point);
		}
	}
}
